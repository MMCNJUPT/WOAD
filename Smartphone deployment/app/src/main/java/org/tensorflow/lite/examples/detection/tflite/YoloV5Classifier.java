...
//            Interpreter interpreter = new Interpreter(modelString, options);
...
//                if (isGPU) {
        ...
        d.INPUT_SIZE = inputSize*2;
        d.imgData = ByteBuffer.allocateDirect(1 * 614400 * 3 * numBytesPerChannel);
        d.imgData.order(ByteOrder.nativeOrder());
        d.intValues = new int[614400];//

//        d.output_box = (int) ((Math.pow((inputSize / 32), 2) + Math.pow((inputSize / 16), 2) + Math.pow((inputSize / 8), 2)) * 3);
        d.output_box = (int) (18900);
...
//        d.outData = ByteBuffer.allocateDirect(d.output_box * (numClass + 5) * numBytesPerChannel);
        d.outData = ByteBuffer.allocateDirect(1209600);
...
//    private int[] ANCHORS;
    ...
    private int[] intValues=new int[614400];
...
//        System.out.println(intValues.length);
        ...
        for (int i = 0; i < 480; ++i) {
            for (int j = 0; j < 1280; ++j) {
                int pixelValue = intValues[i * 1280 + j];
                if (isModelQuantized) {
                    // Quantized model
    ...
    public ArrayList<Recognition> recognizeImage(Bitmap bitmap) {
        ByteBuffer byteBuffer_ = convertBitmapToByteBuffer(bitmap);   //把bitmap存入imgData中
//        System.out.println(imgData.capacity());
//        System.out.println(outData.capacity());
...
//        Log.d("YoloV5Classifier", "mObjThresh: " + getObjThresh());
        ...
        Object[] inputArray = {imgData};    //全局变量imgData
...
//                        Float.toString(xPos) + ',' + yPos + ',' + w + ',' + h);
                ...
                final RectF rect =
                        new RectF(
                                (float)Math.max(0, (xPos - w / 2)/1.3),
                                (float)Math.max(0, (yPos - h / 2)/1.73),
                                (float)Math.min(639, (xPos + w / 2)/1.3),
                                (float)Math.min(639, (yPos + h / 2)/1.73));
